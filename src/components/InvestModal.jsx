...
function CoinAvatar({ coin, marketData, size = 40 }) {
  const [err, setErr] = useState(false)
  const src = getCoinImageUrl(coin.id, marketData?.image)
  if (!src || err) {
    return (
      <div
...
export default function InvestModal({ coin, marketData, wallet, onClose }) {
  const { user } = useAuth()
  ...
  const price = marketData?.current_price || 0
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-3">
            <CoinAvatar coin={coin} marketData={marketData} />
            <div>
          ...
          <>
            <div className="flex justify-between text-sm mb-4">
              ...
              <span className="text-text-primary font-semibold">
                {price > 0 ? formatUSD(price) : 'Loading...'}
              </span>
            </div>
            ...
            <div className="mt-5 bg-root-bg border border-card-border rounded-lg p-4 text-sm space-y-2">
              <div className="flex justify-between">
                ...
                <span className="text-text-primary font-semibold">
                  {receiveQty > 0 ? receiveQty.toFixed(6) : '—'} {coin.symbol}
                </span>
              </div>
              <div className="flex justify-between">
            ...
            <div className="mt-4 bg-primary-blue/5 border border-primary-blue/20 rounded-lg px-4 py-3 text-xs text-text-muted">
              Payment via <span className="text-text-primary font-medium">{wallet.network}</span> network.
            </div>
        ...
        {step === 2 && (
          <>
            <div className="bg-loss/10 border border-loss/30 rounded-lg px-4 py-3 text-xs text-loss mb-4">
              Only send <span className="font-semibold">{wallet.network}</span> to this address. Sending any other asset or using the wrong network will result in permanent loss.
            </div>
            ...
            <div className="bg-root-bg border border-card-border rounded-lg p-4 text-sm space-y-2 mb-4">
              ...
              <div className="flex justify-between">
                <span className="text-text-muted">You will receive</span>
                <span className="text-text-primary font-semibold">
                  {receiveQty.toFixed(6)} {coin.symbol}
                </span>
              </div>
            </div>
        ...
        {step === 3 && (
          <div className="text-center py-4">
            ...
            <div className="text-text-muted text-sm mb-5">
              We will verify your transaction and credit your account within 24 hours.
            </div>
